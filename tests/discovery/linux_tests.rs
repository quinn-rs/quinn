...
mod linux_tests {
    ...
    fn test_linux_discovery_creation() {
        let discovery = LinuxDiscovery::new(Duration::from_secs(60));
    ...
    #[test]
    fn test_linux_discover_interfaces() {
        let discovery = LinuxDiscovery::new(Duration::from_secs(60));
    ...
    #[test]
    fn test_linux_get_default_route() {
        let discovery = LinuxDiscovery::new(Duration::from_secs(60));
    ...
    #[test]
    fn test_linux_cache_refresh() {
        let mut discovery = LinuxDiscovery::new(Duration::from_millis(1));
    ...
    #[test]
    fn test_linux_interface_properties() {
        let discovery = LinuxDiscovery::new(Duration::from_secs(60));
    ...
    #[test]
    fn test_linux_netlink_error_handling() {
        // This test verifies that the Netlink error handling works correctly
        // We can't easily simulate Netlink errors, so we just check that the
...
// Always compile this test, even on non-Linux platforms
#[test]
fn test_linux_discovery_mock() {
    // This test ensures we have a way to test Linux discovery on non-Linux platforms
