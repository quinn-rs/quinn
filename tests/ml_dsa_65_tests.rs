...
mod ml_dsa_65_tests {
    ...
    fn test_ml_dsa_65_signature_size() {
        ...
        let ml_dsa = MlDsa65::new();
        let (_, secret_key) = ml_dsa.generate_keypair()
    ...
    fn test_ml_dsa_65_verify_wrong_message() {
        ...
        let is_valid = ml_dsa.verify(&public_key, wrong_message, &signature)
    ...
    fn test_ml_dsa_65_verify_wrong_key() {
        ...
        let ml_dsa = MlDsa65::new();
        let (_, secret_key1) = ml_dsa.generate_keypair()
            .expect("Failed to generate keypair 1");
        let (public_key2, _) = ml_dsa.generate_keypair()
            .expect("Failed to generate keypair 2");

        let message = b"Test message";
        let signature = ml_dsa.sign(&secret_key1, message)
            .expect("Failed to sign message");

        let is_valid = ml_dsa.verify(&public_key2, message, &signature)
            .expect("Verification should complete");
    ...
    fn test_ml_dsa_65_deterministic_signing() {
        ...
        let signature1 = ml_dsa.sign(&secret_key, message)
            .expect("Failed to sign message 1");
        let signature2 = ml_dsa.sign(&secret_key, message)
        ...
        let (public_key, _) = ml_dsa2.generate_keypair()
    ...
    fn test_ml_dsa_65_public_key_serialization() {
        ...
        let ml_dsa = MlDsa65::new();
        let (public_key, _) = ml_dsa.generate_keypair()
            .expect("Failed to generate keypair");

        let pub_key_bytes = public_key.as_bytes().to_vec();
        
        // Create new public key from bytes
        let restored_key = MlDsaPublicKey::from_bytes(&pub_key_bytes)
    ...
    fn test_ml_dsa_65_signature_serialization() {
        ...
        let restored_sig = MlDsaSignature::from_bytes(&sig_bytes)
            .expect("Failed to restore signature");
    ...
    fn test_ml_dsa_65_corrupted_signature() {
        ...
        let corrupted_sig = MlDsaSignature::from_bytes(&corrupted_bytes)
            .expect("Should create signature object");

        let is_valid = ml_dsa.verify(&public_key, message, &corrupted_sig)
            .expect("Verification should complete");
    ...
    fn test_ml_dsa_65_stress_multiple_operations() {
        ...
        for i in 0..10 {
            ...
            let signature = ml_dsa.sign(&secret_key, message.as_bytes())
                .expect(&format!("Failed to sign message {}", i));

            let is_valid = ml_dsa.verify(&public_key, message.as_bytes(), &signature)
