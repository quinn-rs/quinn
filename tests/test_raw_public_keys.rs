...
fn test_certificate_type_negotiation() {
    ...
    let remote_client_types = Some(CertificateTypeList::new(vec![
        CertificateType::RawPublicKey,
        CertificateType::X509,
    ]).unwrap());
    
    let remote_server_types = Some(CertificateTypeList::new(vec![
        CertificateType::RawPublicKey,
    ]).unwrap());
    ...
    assert_eq!(negotiation_result.client_cert_type, CertificateType::RawPublicKey);
    assert_eq!(negotiation_result.server_cert_type, CertificateType::RawPublicKey);
}
...
fn test_certificate_type_preferences() {
    ...
    assert_eq!(prefer_rpk.client_types.most_preferred(), CertificateType::RawPublicKey);
}
...
fn test_negotiation_caching() {
    ...
    assert_eq!(final_stats.cache_misses, initial_cache_misses + 1); // Only first was a miss
}
