...
async fn test_concurrent_creation_safety() {
    ...
    println!("✓ Concurrent creation test: {}/{} succeeded", successful, NUM_CONCURRENT);
}
...
async fn test_input_sanitization() {
    ...
    assert!(duration < Duration::from_secs(30), "Large config processing took too long");
...
mod specific_regression_tests {
            ...
            Ok(endpoint) => {
                ...
                if let Some(quinn_ep) = endpoint.get_quinn_endpoint() {
                    if let Ok(addr) = quinn_ep.local_addr() {
                        ...
                        assert_eq!(addr.ip().to_string(), "0.0.0.0", "Should bind to all interfaces");
    ...
    async fn test_commit_a7d1de11_robust_error_handling() {
        ...
        let problematic_config = NatTraversalConfig {
            role: EndpointRole::Server { can_coordinate: false }, // Server that can't coordinate
