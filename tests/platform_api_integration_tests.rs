        ...
        match discovery.start_scan() {
            Ok(_) => {
                ...
                if let Some(interfaces) = discovery.check_scan_complete() {
                    ...
                    assert!(!interfaces.is_empty(), "Windows should have at least one network interface");
                    
                    // Check that interfaces have valid data
                    for interface in interfaces {
                        assert!(!interface.name.is_empty(), "Interface name should not be empty");
                        assert!(!interface.addresses.is_empty(), "Interface should have at least one address");
                        
                        println!("Windows interface: {} with {} addresses", 
...
mod linux_tests {
        ...
        match discovery.start_scan() {
            Ok(_) => {
                ...
                if let Some(interfaces) = discovery.check_scan_complete() {
                    ...
                    assert!(!interfaces.is_empty(), "Linux should have at least one network interface");
                    ...
                    for interface in interfaces {
                        assert!(!interface.name.is_empty(), "Interface name should not be empty");
                        println!("Linux interface: {} with {} addresses, up: {}", 
                            interface.name, interface.addresses.len(), interface.is_up);
                    }
                } else {
    ...
    fn test_linux_proc_filesystem_access() {
        ...
        match std::fs::read_to_string("/proc/net/dev") {
            Ok(content) => {
                assert!(content.contains("lo:"), "/proc/net/dev should contain loopback interface");
            }
    ...
    fn test_linux_netlink_monitoring() {
        ...
        match discovery.initialize_netlink_socket() {
            ...
            Err(e) => {
                ...
                println!("Netlink initialization failed (may be normal on CI): {:?}", e);
            }
        }
    }
...
mod macos_tests {
        ...
        match discovery.start_scan() {
            Ok(_) => {
                ...
                if let Some(interfaces) = discovery.check_scan_complete() {
                    ...
                    assert!(!interfaces.is_empty(), "macOS should have at least one network interface");
                    ...
                    for interface in interfaces {
                        assert!(!interface.name.is_empty(), "Interface name should not be empty");
                        println!("macOS interface: {} with {} addresses, wireless: {}", 
                            interface.name, interface.addresses.len(), interface.is_wireless);
                    }
                } else {
    ...
    fn test_macos_scf_dynamic_store() {
        ...
        match discovery.create_dynamic_store() {
            Ok(_) => {
                ...
                assert!(discovery.dynamic_store.is_some(), "Dynamic store should be initialized");
            }
            Err(e) => {
                // Might fail on some CI environments
                println!("Dynamic store creation failed (may be normal on CI): {:?}", e);
            }
        }
    }
    ...
    fn test_macos_framework_availability() {
        ...
        for framework in &frameworks {
            assert!(
                ...
                "Required framework {} should exist", framework
            );
        }
    }
    ...
    fn test_macos_network_change_monitoring() {
        ...
        match discovery.setup_network_monitoring() {
            ...
            Err(e) => {
                println!("Network monitoring setup failed (may be normal on CI): {:?}", e);
            }
        }
    }
