...
async fn test_address_spoofing_prevention() {
    ...
    let client_node = Arc::new(
        QuicP2PNode::new(client_config).await.expect("Failed to create client node")
    );
    ...
    let attacker_node = Arc::new(
        QuicP2PNode::new(attacker_config).await.expect("Failed to create attacker node")
    );
...
async fn test_no_information_leaks() {
    ...
    assert!(time_diff < Duration::from_nanos(1000), "Address type detection should be constant time");
    ...
    for (octets, expected_private) in test_addresses {
        ...
        assert_eq!(is_private, expected_private, "Private address detection failed for {:?}", addr);
    }
}
...
async fn test_penetration_scenarios() {
    ...
    assert_eq!(attacker_stats.active_connections, 1, "Attacker should only see bootstrap");
...
async fn test_symmetric_nat_prediction_defense() {
    ...
    assert!(port_range > 100, "Port allocation should have good diversity");
}
