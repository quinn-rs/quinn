...
use super::utils;
use ant_quic::high_level::{Endpoint, Connection};
use anyhow::Result;
...
async fn test_stream_operations(
    ...
    for i in 0..10 {
        match timeout(Duration::from_secs(2), conn.open_bi()).await {
            Ok(Ok((mut send, mut recv))) => {
                send.write_all(format!("Bidi stream {} test", i).as_bytes()).await?;
                send.finish()?;
    ...
    metrics.insert("bidi_streams_ms".to_string(), bidi_duration.as_millis() as f64);
    ...
    for i in 0..10 {
        match timeout(Duration::from_secs(2), conn.open_uni()).await {
            Ok(Ok(mut send)) => {
                send.write_all(format!("Uni stream {} test", i).as_bytes()).await?;
                send.finish()?;
                uni_count += 1;
            }
            Ok(Err(e)) => {
    ...
    metrics.insert("uni_streams_ms".to_string(), uni_duration.as_millis() as f64);
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            send.write_all(format!("Concurrent stream {}", i).as_bytes()).await?;
            send.finish()?;
            Ok::<_, anyhow::Error>(())
        });
        handles.push(handle);
    ...
    metrics.insert("concurrent_ms".to_string(), concurrent_duration.as_millis() as f64);
...
async fn test_flow_control(
    ...
    metrics.insert("flow_control_ms".to_string(), flow_duration.as_millis() as f64);
    ...
    metrics.insert("conn_flow_ms".to_string(), conn_flow_duration.as_millis() as f64);
...
async fn test_congestion_control(
    ...
    for i in 0..num_bursts {
        ...
        match timeout(Duration::from_secs(5), send.write_all(&burst_data)).await {
            Ok(Ok(_)) => {
                ...
                let throughput = (burst_size as f64 * 8.0) / 
                    (burst_duration.as_secs_f64() * 1_000_000.0);
    ...
    if !throughputs.is_empty() {
        ...
        metrics.insert("throughput_variance".to_string(), 
...
async fn test_loss_recovery(
    ...
    metrics.insert("ack_test_ms".to_string(), ack_test_duration.as_millis() as f64);
...
mod tests {
    ...
    async fn test_transport_features_framework() {
        ...
        let result = test_stream_operations(&endpoint, "test.example.com:443").await;
