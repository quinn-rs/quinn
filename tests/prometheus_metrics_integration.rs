...
async fn test_prometheus_exporter_basic_functionality() {
    ...
    let exporter =
        PrometheusExporter::new(collector).expect("Failed to create Prometheus exporter");
...
async fn test_metrics_server_creation_and_shutdown() {
    ...
    let collector = Arc::new(MetricsCollector::new());
    let exporter =
        Arc::new(PrometheusExporter::new(collector).expect("Failed to create Prometheus exporter"));
...
async fn test_metrics_persistence_across_updates() {
    let collector = Arc::new(MetricsCollector::new());
    let exporter =
        PrometheusExporter::new(collector.clone()).expect("Failed to create Prometheus exporter");
