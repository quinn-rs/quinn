...
fn test_varint_edge_cases() {
    let test_values = vec![
        0u64, 1u64, 63u64, 64u64, 16383u64, 16384u64,
        1073741823u64, 1073741824u64, 4611686018427387903u64,
    ];

    for &value in &test_values {
        let varint = VarInt::from_u64(value).expect("VarInt creation should succeed");
        assert_eq!(varint.into_inner(), value, "VarInt roundtrip failed for {}", value);
    }
