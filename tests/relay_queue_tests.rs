...
use ant_quic::{
    nat_traversal_api::{PeerId, NatTraversalConfig, NatTraversalEndpoint, EndpointRole, NatTraversalError},
    VarInt,
};
...
mod nat_traversal_api_tests {
    ...
    fn test_endpoint_role_matching() {
        ...
        // Test server roles
        let server_no_relay = EndpointRole::Server { can_coordinate: false };
        let server_with_relay = EndpointRole::Server { can_coordinate: true };
        
        assert!(matches!(server_no_relay, EndpointRole::Server { can_coordinate: false }));
        assert!(matches!(server_with_relay, EndpointRole::Server { can_coordinate: true }));
    ...
    #[test]
    fn test_nat_traversal_endpoint_creation_without_bootstrap() {
        ...
        let result = NatTraversalEndpoint::new(config, None);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("bootstrap node"));
    }

    #[test]
    fn test_nat_traversal_endpoint_creation_bootstrap_role() {
    ...
    #[test]
    fn test_nat_traversal_endpoint_creation_with_bootstrap() {
        let bootstrap_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(203, 0, 113, 1)), 8080);
        let config = NatTraversalConfig {
            role: EndpointRole::Client,
            bootstrap_nodes: vec![bootstrap_addr],
    ...
    fn test_bootstrap_node_management() {
        ...
        if let Ok(endpoint) = NatTraversalEndpoint::new(config, None) {
...
mod nat_traversal_integration_tests {
    ...
    #[test]
    fn test_nat_traversal_initiation() {
        let bootstrap_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(203, 0, 113, 1)), 8080);
        let config = NatTraversalConfig {
            role: EndpointRole::Bootstrap,
            bootstrap_nodes: vec![bootstrap_addr],
    ...
    #[test]
    fn test_polling_without_active_sessions() {
        let config = NatTraversalConfig {
            role: EndpointRole::Bootstrap,
            bootstrap_nodes: vec![],
            ..NatTraversalConfig::default()
    ...
    #[test]
    fn test_statistics_without_activity() {
        let config = NatTraversalConfig {
            role: EndpointRole::Bootstrap,
            bootstrap_nodes: vec![],
            ..NatTraversalConfig::default()
...
mod performance_tests {
    ...
    fn bench_configuration_creation() {
        ...
        for i in 0..1000 {
            let config = NatTraversalConfig {
                role: if i % 3 == 0 { EndpointRole::Bootstrap } else { EndpointRole::Client },
                bootstrap_nodes: vec![
                    SocketAddr::new(IpAddr::V4(Ipv4Addr::new(203, 0, 113, i as u8)), 8080)
                ],
...
mod relay_functionality_tests {
    ...
    fn test_endpoint_roles_for_relay() {
        ...
        let server_no_relay = EndpointRole::Server { can_coordinate: false };
        let server_relay = EndpointRole::Server { can_coordinate: true };
        
        assert!(matches!(server_no_relay, EndpointRole::Server { can_coordinate: false }));
        assert!(matches!(server_relay, EndpointRole::Server { can_coordinate: true }));
    ...
    #[test]
    fn test_invalid_configuration_scenarios() {
        ...
        let result = NatTraversalEndpoint::new(invalid_client_config, None);
        assert!(result.is_err());
        ...
        let _result = NatTraversalEndpoint::new(zero_values_config, None);
    }
}