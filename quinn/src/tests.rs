...
async fn split_accept_wait_idle_and_open_connections() {
    ...
    assert!(
        timeout(Duration::from_millis(50), server.wait_idle())
            .await
            .is_err()
    );
    ...
    let client_conn = timeout(Duration::from_secs(5), connect)
        .await
        .unwrap()
        .unwrap()
        .unwrap();
    let server_conn = timeout(Duration::from_secs(5), accept)
        .await
        .unwrap()
        .unwrap()
        .unwrap();
    ...
    let _ = timeout(Duration::from_secs(5), client_conn.closed())
        .await
        .unwrap();
    let _ = timeout(Duration::from_secs(5), server_conn.closed())
        .await
        .unwrap();
...
async fn split_accept_close_during_pending_accept() {
    ...
    match timeout(Duration::from_secs(5), accept)
        .await
        .unwrap()
        .unwrap()
    {
        Ok(conn) => {
            let err = timeout(Duration::from_secs(5), conn.closed())
                .await
                .unwrap();
    ...
    match timeout(Duration::from_secs(5), connect)
        .await
        .unwrap()
        .unwrap()
    {
