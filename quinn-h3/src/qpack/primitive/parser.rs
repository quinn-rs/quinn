...
impl<'a> Parser<'a> {
    ...
    fn next_byte(&mut self) -> Option<usize> {
        if self.buf.has_remaining() {
            Some(self.buf.get_u8() as usize)
        } else {
            None
        }
    }

    pub fn integer(&mut self, starter: StarterByte) -> Result<usize, Error> {
        let byte = starter
            .byte
    ...
    fn var_len_integer(&mut self, starter: StarterByte) -> Result<usize, Error> {
        let mut value = starter.mask;

        let mut count = 0usize;
        loop {
            let byte = self.next_byte().ok_or(Error::TooShortBufferForInteger)?;
    ...
    pub fn string(&mut self, starter: StarterByte) -> Result<Vec<u8>, Error> {
        let byte = starter
            .byte
            .or_else(|| self.next_byte())
            .ok_or(Error::NoByteForIntegerLength)?;
        ...
        let str_len = if starter.prefix > 1 {
            self.integer(
                StarterByte::valued(starter.prefix - 1, byte).expect("valid starter byte"),
            )?
        } else {
        ...
        if huffman_encoded {
            let decoded: Result<Vec<_>, HuffmanDecodingError> = str_bytes.hpack_decode().collect();
            str_bytes = decoded.map_err(|x| Error::InvalidHuffmanStringDecoding(x))?;
        }
...
mod tests {
    ...
    fn test_read_integer_fit_in_prefix() {
        ...
        let res = parser.integer(StarterByte::prefixed(7).expect("valid starter byte"));
    ...
    fn test_read_integer_too_large_for_prefix() {
        ...
        let mut cursor = Cursor::new(&bytes);
        let mut parser = Parser::new(&mut cursor);
        let res = parser.integer(StarterByte::prefixed(5).expect("valid starter byte"));
    ...
    fn test_read_invalid_var_len_integer() {
        ...
        let mut cursor = Cursor::new(&bytes);
        let mut parser = Parser::new(&mut cursor);
        let res = parser.integer(StarterByte::prefixed(2).expect("valid starter byte"));
    ...
    fn test_read_integer_with_ahead_byte() {
        ...
        let mut cursor = Cursor::new(&bytes);
        let mut parser = Parser::new(&mut cursor);
        let res = parser.integer(StarterByte::valued(5, first_byte).expect("valid starter byte"));
    ...
    fn test_read_ascii_string() {
        ...
        let bytes: [u8; 14] = [
            // not huffman, size
            13, // bytes
            'T' as u8, 'e' as u8, 's' as u8, 't' as u8, 'i' as u8, 'n' as u8, 'g' as u8, ' ' as u8,
            'a' as u8, 's' as u8, 'c' as u8, 'i' as u8, 'i' as u8,
        ];
