...
impl HuffmanDecoder {
    ...
    fn check_eof(&self,
        ...
        else if (range.byte + 1) as usize == input.len() {
...
pub mod huffman_code {
    ...
    macro_rules! huffman {
        ...
        [ $( $name:ident => ( $($value:tt)* ); )* ] => {
            $( pub const $name: HuffmanDecoder = huffman!( $( $value )* ); )*
        };
    }
    ...
    huffman![
        ...
        END23B_1100 => (lookup: 3, [
                        192, 193, 200, 201, 202, 205, 210, 213,]);
        END23B_1101 => (lookup: 3, [
                        218, 219, 238, 240, 242, 243, 255, => END27A_111,]);
        END27A_111 => (203, 204);
        END23B_1110 => (lookup: 4, [
                        211, 212, 214, 221, 222, 223, 241, 244, 
                        245, 246, 247, 248, 250, 251, 252, 253,]);
        END23B_1111 => (lookup: 4, [
                        254, 
                        => END27B_0001,
                        => END27B_0010,
                        => END27B_0011,
    ...
    fn test_decode_all_code_joined() {
        let bytes = [
            ...
            0b11111111, 0b11111111, 0b11111011, (0b10 << 6)
                ...
                + 0b111111, 0b11111111, 0b11111111, 0b11111111,
            ].to_vec();
