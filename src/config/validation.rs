...
pub(crate) fn validate_socket_addr(addr: &SocketAddr, context: &str) -> ValidationResult<()> {
    ...
    match addr.ip() {
        std::net::IpAddr::V4(ipv4) => {
            if ipv4.is_unspecified() {
                return Err(ConfigValidationError::InvalidAddress(
                    format!("{}: IPv4 address cannot be unspecified (0.0.0.0)", context)
                ));
            }
            if ipv4.is_broadcast() {
                return Err(ConfigValidationError::InvalidAddress(
                    format!("{}: IPv4 address cannot be broadcast (255.255.255.255)", context)
                ));
            }
            if ipv4.is_multicast() {
                return Err(ConfigValidationError::InvalidAddress(
                    format!("{}: IPv4 address cannot be multicast", context)
                ));
            }
        }
        std::net::IpAddr::V6(ipv6) => {
            if ipv6.is_unspecified() {
                return Err(ConfigValidationError::InvalidAddress(
                    format!("{}: IPv6 address cannot be unspecified (::)", context)
                ));
            }
            if ipv6.is_multicast() {
                return Err(ConfigValidationError::InvalidAddress(
                    format!("{}: IPv6 address cannot be multicast", context)
                ));
            }
        }
    }
    ...
    if addr.port() == 0 {
        return Err(ConfigValidationError::InvalidAddress(
            format!("{}: port cannot be 0", context)
        ));
    }
    ...
    if addr.port() < 1024 && !is_allowed_privileged_port(addr.port()) {
        return Err(ConfigValidationError::InvalidAddress(
            format!("{}: port {} is a privileged port, ensure proper permissions", context, addr.port())
        ));
    }
...
pub(crate) fn validate_duration(duration: Duration, min: Duration, max: Duration, context: &str) -> ValidationResult<()> {
    if duration < min {
        return Err(ConfigValidationError::ValueOutOfRange(
            format!("{}: duration {:?} is less than minimum {:?}", context, duration, min)
        ));
    }
    
    if duration > max {
        return Err(ConfigValidationError::ValueOutOfRange(
            format!("{}: duration {:?} is greater than maximum {:?}", context, duration, max)
        ));
    }
    ...
    T: PartialOrd + std::fmt::Display + Copy,
{
    if value < min {
        return Err(ConfigValidationError::ValueOutOfRange(
            format!("{}: value {} is less than minimum {}", context, value, min)
        ));
    }
    
    if value > max {
        return Err(ConfigValidationError::ValueOutOfRange(
            format!("{}: value {} is greater than maximum {}", context, value, max)
        ));
    }
...
pub(crate) fn validate_bootstrap_nodes(nodes: &[SocketAddr]) -> ValidationResult<()> {
    ...
    for (i, node) in nodes.iter().enumerate() {
        if !seen.insert(node) {
            return Err(ConfigValidationError::InvalidBootstrapNode(
                format!("Duplicate bootstrap node at index {}: {}", i, node)
            ));
        }
