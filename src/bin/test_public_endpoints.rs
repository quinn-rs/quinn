...
async fn test_endpoint(
    ...
    let connect_result = timeout(Duration::from_secs(test_config.timeout_seconds), async move {
        connecting.await
    }).await;
    ...
    for chunk in endpoints_to_test.chunks(args.parallel) {
        ...
        for endpoint in chunk {
            ...
            let handle = tokio::spawn(async move {
                test_endpoint(&endpoint, client_config, &test_config).await
            });
            handles.push(handle);
        }
...
fn generate_markdown_report(results: &ValidationResults) -> String {
    ...
    report.push_str(&format!("**ant-quic Version**: {}\n", results.metadata.ant_quic_version));
    ...
    report.push_str(&format!("- **Average Handshake Time**: {:.1}ms\n", results.summary.average_handshake_time));
    report.push_str(&format!("- **Protocols Seen**: {}\n\n", results.summary.protocols_seen.join(", ")));
    ...
    for result in &results.endpoints {
        ...
        let handshake = result.handshake_time_ms
            .map(|ms| format!("{}ms", ms))
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Print summary
    println!("\nValidation Summary:");
    println!("Total endpoints tested: {}", results.summary.total_endpoints);
    println!("Successful connections: {} ({:.1}%)", 
        results.summary.passed_endpoints, 
        results.summary.success_rate
    );
    println!("Average handshake time: {:.1}ms", results.summary.average_handshake_time);
