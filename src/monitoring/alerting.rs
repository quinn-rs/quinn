...
impl ProductionAlertManager {
    ...
    pub async fn evaluate_nat_result(&self, result: &NatTraversalResult) -> Result<(), MonitoringError> {
        ...
        metrics.insert("duration_ms".to_string(), result.duration.as_millis() as f64);
        metrics.insert("success".to_string(), if result.success { 1.0 } else { 0.0 });
        
        let perf = &result.performance_metrics;
        metrics.insert("connection_time_ms".to_string(), perf.connection_time_ms as f64);
    ...
    pub async fn get_status(&self) -> String {
        ...
        format!("Active: {}, Suppressed: {}", active_alerts, suppressed_alerts)
    }
    ...
    async fn process_alert(&self, alert: Alert) -> Result<(), MonitoringError> {
        ...
        info!("Processed alert: {} (severity: {:?})", alert.title, alert.severity);
        Ok(())
    }
...
impl AlertRule {
    ...
    fn default_success_rate_rule() -> Self {
        Self {
            ...
            annotations: HashMap::from([
                ("summary".to_string(), "NAT traversal success rate below 80%".to_string()),
                ("runbook".to_string(), "https://docs.example.com/runbooks/nat-success-rate".to_string()),
            ]),
        }
    }
    ...
    fn default_latency_rule() -> Self {
        Self {
            ...
            annotations: HashMap::from([
                ("summary".to_string(), "NAT traversal P95 latency above 5s".to_string()),
            ]),
        }
    }
    ...
    fn default_error_rate_rule() -> Self {
        Self {
            ...
            annotations: HashMap::from([
                ("summary".to_string(), "NAT traversal error rate above 10%".to_string()),
...
impl AlertRuleEngine {
    ...
    async fn evaluate_rules(&self, context: &AlertEvaluationContext) -> Result<(), MonitoringError> {
    ...
    async fn evaluate_scheduled_rules(&self, context: &AlertEvaluationContext) -> Result<(), MonitoringError> {
    ...
    async fn evaluate_single_rule(&self, rule: &AlertRule, context: &AlertEvaluationContext) -> Result<(), MonitoringError> {
        let should_alert = match &rule.condition {
            AlertCondition::Threshold { metric, operator, value, duration: _ } => {
                self.evaluate_threshold_condition(metric, operator, *value, context).await?
            }
            AlertCondition::RateOfChange { metric, rate_threshold, .. } => {
                self.evaluate_rate_condition(metric, *rate_threshold, context).await?
            }
            AlertCondition::Anomaly { metric, sensitivity, .. } => {
                self.evaluate_anomaly_condition(metric, *sensitivity, context).await?
            }
            AlertCondition::Expression { expression, .. } => {
                self.evaluate_expression_condition(expression, context).await?
            }
        };
    ...
    async fn evaluate_rate_condition(&self, _metric: &str, _rate_threshold: f64, _context: &AlertEvaluationContext) -> Result<bool, MonitoringError> {
        // Would calculate rate of change
        Ok(false)
    }
    
    async fn evaluate_anomaly_condition(&self, _metric: &str, _sensitivity: f64, _context: &AlertEvaluationContext) -> Result<bool, MonitoringError> {
        // Would use anomaly detection algorithms
        Ok(false)
    }
    
    async fn evaluate_expression_condition(&self, _expression: &str, _context: &AlertEvaluationContext) -> Result<bool, MonitoringError> {
        // Would evaluate complex expressions
        Ok(false)
    }
    
    async fn create_alert_from_rule(&self, rule: &AlertRule, context: &AlertEvaluationContext) -> Alert {
        Alert {
            id: format!("{}_{}", rule.id, context.timestamp.duration_since(SystemTime::UNIX_EPOCH).unwrap().as_secs()),
...
impl NotificationDispatcher {
    ...
    fn select_channels_for_alert(&self, alert: &Alert) -> Vec<String> {
        ...
        match alert.severity {
            ...
            AlertSeverity::Critical => vec![self.config.default_channel.clone(), "pagerduty".to_string()],
            AlertSeverity::Fatal => vec![self.config.default_channel.clone(), "pagerduty".to_string(), "sms".to_string()],
        }
    }
    ...
    async fn send_to_channel(&self, channel: &NotificationChannel, alert: &Alert) -> Result<(), MonitoringError> {
        match channel {
    ...
    async fn send_slack_notification(&self, _webhook_url: &str, _channel: &str, alert: &Alert) -> Result<(), MonitoringError> {
        info!("Sending Slack notification for alert: {}", alert.title);
        // Would implement actual Slack webhook call
        Ok(())
    }
    
    async fn send_email_notification(&self, _recipients: &[String], alert: &Alert) -> Result<(), MonitoringError> {
        info!("Sending email notification for alert: {}", alert.title);
        // Would implement actual email sending
        Ok(())
    }
    
    async fn send_pagerduty_notification(&self, _service_key: &str, alert: &Alert) -> Result<(), MonitoringError> {
        info!("Sending PagerDuty notification for alert: {}", alert.title);
        // Would implement actual PagerDuty API call
        Ok(())
    }
    
    async fn send_webhook_notification(&self, _url: &str, _headers: &HashMap<String, String>, alert: &Alert) -> Result<(), MonitoringError> {
        info!("Sending webhook notification for alert: {}", alert.title);
        // Would implement actual HTTP webhook call
        Ok(())
    }
    
    async fn send_sms_notification(&self, _numbers: &[String], alert: &Alert) -> Result<(), MonitoringError> {
...
impl EscalationManager {
    ...
    async fn escalate_alert(&self, alert: &Alert) -> Result<(), MonitoringError> {
        info!("Escalating alert: {} (severity: {:?})", alert.title, alert.severity);
...
impl SuppressionEngine {
    ...
    async fn cleanup_expired_suppressions(&self) -> Result<(), MonitoringError> {
        ...
        suppressions.retain(|_, suppression| {
            now.duration_since(suppression.created_at).unwrap_or_default() < suppression.rule.duration
        });
...
mod tests {
    ...
    async fn test_threshold_evaluation() {
        ...
        let should_alert = engine.evaluate_threshold_condition(
            "nat_success_rate",
