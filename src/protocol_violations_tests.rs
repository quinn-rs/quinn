...
fn test_frame_in_wrong_packet_type() {
        ...
        let result = validate_frame_in_packet_type(
            FrameType::NEW_TOKEN,
            PacketType::Initial
        );
...
fn test_ack_frame_in_wrong_space() {
        ...
        let result = validate_ack_frame_space(
            SpaceId::Initial,
            SpaceId::Handshake
        );
        assert!(result.is_err());
...
fn test_coalesced_packet_violations() {
        ...
        let result = validate_coalesced_packet_order(
            vec![PacketType::Handshake, PacketType::Initial]
        );
        assert!(result.is_err());
        assert_eq!(result.unwrap_err().code, Code::PROTOCOL_VIOLATION);
}
