...
use crate::workflow::{StageId, WorkflowError, WorkflowId, WorkflowMetrics, WorkflowStatus};
...
impl WorkflowMonitor {
    ...
    pub async fn get_workflow_summary(&self) -> WorkflowSummary {
        ...
        let total_duration: Duration = metrics
            ...
            .filter_map(|data| {
                data.end_time
                    .map(|end| end.duration_since(data.start_time).unwrap_or_default())
            })
            .sum();
    ...
    async fn process_event(&self, event: MonitoringEvent) -> Result<(), WorkflowError> {
        ...
        // Update metrics
        match &event {
            MonitoringEvent::WorkflowStarted {
                workflow_id,
                definition_id,
                timestamp,
            } => {
                    ...
                    *workflow_id,
                    WorkflowMonitoringData {
                        ...
                        status: WorkflowStatus::Running {
                            current_stage: StageId("init".to_string()),
                        },
            ...
            MonitoringEvent::WorkflowCompleted {
                workflow_id,
                duration,
                success,
                timestamp,
            ...
            MonitoringEvent::StageCompleted {
                workflow_id,
                stage_id: _,
                duration,
                timestamp: _,
    ...
    async fn check_alerts(&self) {
        ...
        if system.memory_usage_mb > self.config.alert_config.memory_threshold_mb {
            self.trigger_alert(
                ...
                format!(
                    "Memory usage {}MB exceeds threshold",
    ...
    async fn trigger_alert(
        ...
        for handler in handlers.iter() {
            handler
                .handle_alert(alert_type.clone(), message.clone(), workflow_id)
                .await;
        }
    }
    ...
    async fn cleanup_loop(&self) {
        ...
        loop {
            ...
            // Clean up old events
            let mut history = self.event_history.write().await;
            history.retain(|event| match event {
...
mod tests {
    ...
    async fn test_workflow_monitor() {
        ...
        monitor
            .record_event(MonitoringEvent::WorkflowStarted {
                workflow_id,
        ...
        monitor
            .record_event(MonitoringEvent::StageCompleted {
                workflow_id,
        ...
        monitor
            .record_event(MonitoringEvent::WorkflowCompleted {
                workflow_id,
