...
pub fn parse_structured_fields(format_str: &str, args: &[&dyn std::fmt::Display]) -> Vec<(String, String)> {
...
mod tests {
    ...
    fn test_structured_event_builder() {
        ...
        assert!(event.fields.contains(&("key1".to_string(), "value1".to_string())));
        assert!(event.fields.contains(&("count".to_string(), "42".to_string())));
        assert!(event.fields.contains(&("enabled".to_string(), "true".to_string())));
        assert!(event.fields.contains(&("optional".to_string(), "present".to_string())));
    }
