...
mod test {
    ...
    fn key_derivation() {
        ...
        let hs_secret = hkdf::extract(digest, &HANDSHAKE_SALT, &id.0);
        assert_eq!(&hs_secret[..],
                   [165, 114, 176, 36, 90, 241, 237, 223, 92, 97, 198, 227, 247, 249, 48, 76, 166, 107, 251, 76, 170, 247, 101, 103, 213, 203, 141, 209, 220, 78, 130, 11]);
        
        let client_secret = hkdf::qexpand(digest, &hs_secret, b"client hs", digest.size() as u16);
        assert_eq!(&client_secret[..],
                   [82, 30, 89, 243, 207, 41, 199, 238, 79, 107, 178, 166, 245, 65, 127, 201, 145, 138, 88, 122, 159, 184, 179, 251, 110, 214, 21, 83, 133, 223, 242, 60]);
        let client_state = CryptoState::new(digest, cipher, client_secret);
        assert_eq!(&client_state.key[..],
                   [45, 219, 105, 121, 22, 101, 84, 39, 181, 120, 115, 213, 70, 33, 139, 130]);
        assert_eq!(&client_state.iv[..],
                   [221, 246, 244, 10, 208, 125, 207, 93, 37, 34, 13, 30]);

        let server_secret = hkdf::qexpand(digest, &hs_secret, b"server hs", digest.size() as u16);
        assert_eq!(&server_secret[..],
                   [208, 69, 255, 148, 225, 108, 134, 161, 129, 154, 46, 228, 18, 104, 0, 47, 68, 176, 247, 222, 171, 9, 32, 175, 128, 58, 243, 48, 53, 84, 96, 192]);
        let server_state = CryptoState::new(digest, cipher, server_secret);
        assert_eq!(&server_state.key[..],
                   [241, 170, 185, 157, 7, 96, 82, 218, 49, 104, 148, 167, 2, 97, 14, 164]);
        assert_eq!(&server_state.iv[..],
                   [7, 11, 121, 192, 198, 28, 45, 20, 184, 166, 143, 203]);
    }
}
