...
impl SimulatedExtensionContext {
    ...
    pub fn simulate_receive_preferences(
        ...
        let state = negotiations.get_mut(conn_id)
            .ok_or_else(|| TlsExtensionError::InvalidExtensionData(
                format!("No negotiation state for connection {}", conn_id)
            ))?;
...
impl Rfc7250ClientConfig {
    ...
    pub fn get_client_hello_extensions(&self, conn_id: &str) -> Vec<(u16, Vec<u8>)> {
        let (client_types, server_types) = self.extension_context.simulate_send_preferences(conn_id);
...
pub fn should_use_raw_public_key(
...
impl crate::crypto::Session for ExtensionAwareTlsSession {
    fn initial_keys(&self, dst_cid: &crate::ConnectionId, side: crate::Side) -> crate::crypto::Keys {
    ...
    fn early_crypto(&self) -> Option<(Box<dyn crate::crypto::HeaderKey>, Box<dyn crate::crypto::PacketKey>)> {
    ...
    fn read_handshake(&mut self, buf: &[u8]) -> Result<bool, crate::TransportError> {
        ...
        if result && !self.handshake_complete && !self.is_handshaking() {
            ...
            self.extension_hooks.on_handshake_complete(&self.conn_id, self.is_client);
        }
        
        Ok(result)
    }
    
    fn transport_parameters(&self) -> Result<Option<crate::transport_parameters::TransportParameters>, crate::TransportError> {
    ...
    fn next_1rtt_keys(&mut self) -> Option<crate::crypto::KeyPair<Box<dyn crate::crypto::PacketKey>>> {
        self.inner_session.next_1rtt_keys()
    }
    
    fn is_valid_retry(&self, orig_dst_cid: &crate::ConnectionId, header: &[u8], payload: &[u8]) -> bool {
        self.inner_session.is_valid_retry(orig_dst_cid, header, payload)
    }
    ...
    fn export_keying_material(
        ...
        self.inner_session.export_keying_material(output, label, context)
    }
}
...
impl QuicClientConfig for Rfc7250QuicClientConfig {
    fn start_session(
        ...
        let inner_session = self.base_config.clone().start_session(version, server_name, params)?;
        
        // Create connection ID for this session
        let conn_id = format!("client-{}-{}", server_name, std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
...
impl QuicServerConfig for Rfc7250QuicServerConfig {
    fn start_session(
        ...
        // Create connection ID for this session
        let conn_id = format!("server-{}", std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos());
    ...
    fn retry_tag(&self, version: u32, orig_dst_cid: &crate::ConnectionId, packet: &[u8]) -> [u8; 16] {
...
mod tests {
    ...
    fn test_simulated_negotiation_flow() {
        ...
        client_ctx.simulate_receive_preferences(
            conn_id,
    ...
    fn test_wrapper_configs() {
        let client_config = ClientConfig::builder()
            .dangerous()
            .with_custom_certificate_verifier(Arc::new(crate::crypto::raw_public_keys::RawPublicKeyVerifier::new(Vec::new())))
            .with_no_client_auth();
