...
impl P2PConfigBuilder {
    ...
    pub fn build(&self) -> ConfigResult<P2PConfig> {
        ...
        for (i, node) in self.bootstrap_nodes.iter().enumerate() {
            if !seen.insert(node.address) {
                return Err(ConfigError::InvalidBootstrapNode(
                    format!("Duplicate bootstrap node at index {}: {}", i, node.address)
                ));
            }
        }
        ...
        let keypair = self.keypair.clone().ok_or_else(|| {
            ConfigError::MissingRequiredConfig("Keypair is required".to_string())
        })?;
        ...
        if self.connection_timeout < Duration::from_secs(1) || self.connection_timeout > Duration::from_secs(300) {
            return Err(ConfigError::ValueOutOfRange(
                format!("Connection timeout must be between 1 and 300 seconds, got {:?}", self.connection_timeout)
            ));
        }
        ...
        if self.max_connection_attempts == 0 || self.max_connection_attempts > 10 {
            return Err(ConfigError::ValueOutOfRange(
                format!("Maximum connection attempts must be between 1 and 10, got {}", self.max_connection_attempts)
            ));
        }
        ...
        if self.max_concurrent_connections == 0 || self.max_concurrent_connections > 1000 {
            return Err(ConfigError::ValueOutOfRange(
                format!("Maximum concurrent connections must be between 1 and 1000, got {}", self.max_concurrent_connections)
            ));
        }
...
mod tests {
    ...
    fn test_builder_with_valid_config() {
        ...
        let config = P2PConfigBuilder::new()
            ...
            .with_nat_traversal(true)
            .with_listen_address(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 8000))
            .with_connection_timeout(Duration::from_secs(60))
            .build()
            .unwrap();
        
        assert_eq!(config.bootstrap_nodes.len(), 1);
        assert_eq!(config.bootstrap_nodes[0].address.to_string(), "127.0.0.1:9000");
