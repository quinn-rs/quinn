        ...
        if let Err(conn_err) = result {
            ...
            self.state = match conn_err {
                ...
                ConnectionError::AlreadyConnected => {
                    unreachable!("AlreadyConnected isn't generated by packet processing");
                }
            };
        }
...
pub enum ConnectionError {
    ...
    CidsExhausted,
    /// The connection attempt may be retried, e.g. when using `poll_fn(_)`.
    ///
    /// If the connection has already been established beforehand, return this error instead
...
impl From<ConnectionError> for io::Error {
    fn from(x: ConnectionError) -> Self {
        ...
        let kind = match x {
            ...
            | AlreadyConnected => io::ErrorKind::Other,
        };
        Self::new(kind, x)
    }
}
