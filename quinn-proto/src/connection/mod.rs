...
impl Connection {
        ...
        for &timer in &Timer::VALUES {
            ...
            match timer {
                ...
                Timer::LossDetection => {
                    ...
                    // A packet declared lost by the time threshold triggers no probe, and one
                    // that carried nothing retransmittable queues no data. That can leave
                    // nothing in flight to elicit the ACK that would otherwise arm the timer
    ...
    fn on_ack_received(
        ...
        self.set_loss_detection_timer(now);
        // Acknowledgment is how the set of in-flight packets normally drains, which makes this
        // the main place where the timer's "no ack-eliciting packets in flight" condition can
        // become true
        self.set_blocked_timer(now);
        Ok(())
    }
