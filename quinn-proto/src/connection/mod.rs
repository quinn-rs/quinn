    ...
    fn handle_packet(
        ...
        if let Err(conn_err) = result {
            ...
            self.state = match conn_err {
                ...
                ConnectionError::VersionMismatch(_) => State::Draining,
    ...
    fn process_decrypted_packet(
        ...
        match packet.header {
            ...
            Header::VersionNegotiate { .. } => {
                ...
                let peer_versions = packet
                    .payload
                    .chunks(4)
                    .map(|x| u32::from_be_bytes(x.try_into().expect("Versions not aligned")))
                    .collect::<Vec<_>>();

                let is_supported = peer_versions.iter().any(|version| self.version == *version);
                ...
                Err(ConnectionError::VersionMismatch(peer_versions))
            }
...
pub enum ConnectionError {
    ...
    ///
    /// It includes the list of peer's supported version.
...
impl From<ConnectionError> for io::Error {
    fn from(x: ConnectionError) -> io::Error {
        ...
        let kind = match x {
            ...
            TransportError(_) | VersionMismatch(_) | LocallyClosed => io::ErrorKind::Other,
        };
        io::Error::new(kind, x)
    }
}
