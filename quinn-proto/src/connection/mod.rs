    ...
    S: crypto::Session,
{
    ...
    /// will yield the best performance. Chunk sizes should be no larger than the UDP MTU (in the
    /// 1500 bytes range, unless IPv6 jumbograms are supported). `ordered` will make sure the
    /// returned chunk's offset will have an offset exactly equal to the previously returned offset
    /// plus the previously returned bytes' length.
    ///
    ...
    fn read_crypto(
        ...
        while let Some((_, buf)) = space.crypto_stream.read(usize::MAX, true).unwrap() {
