...
impl Endpoint {
    ...
    fn handle_first_packet(
        ...
        let (retry_src_cid, orig_dst_cid) = if server_config.use_retry {
            match self.decode_retry_token(&dst_cid, &addresses.remote, &token) {
                Ok(Some(orig_dst_cid)) => (Some(dst_cid), orig_dst_cid),
                Err(err) => {
                ...
                Ok(None) => {
                    // This is a first Initial packet, so we must send a Retry.
                    //
    ...
    fn decode_retry_token(
        ...
        let server_config = self.server_config.as_ref().unwrap().clone();
        debug_assert!(server_config.use_retry);
        match RetryToken::from_bytes(&*server_config.token_key, remote, dst_cid, token) {
            Ok(token) if token.issued + server_config.retry_token_lifetime > SystemTime::now() => {
                // Token checks out, no retry.
                Ok(Some(token.orig_dst_cid))
            }
            Err(RetryError::UnknownToken) => {
                ...
                // can't interpret it, so we proceed with Retry to re-validate.
                Ok(None)
            }
            _ => {
                debug!("rejecting invalid stateless retry token");
                Err(TransportError::INVALID_TOKEN(""))
            }
        }
    }
