...
/// 2.77) BBRStartupPacingGain; used in Startup mode for BBR.pacing_gain. <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-05.html#section-2.4-2>
...
pub struct Bbr3 {
    ...
    /// ln(2) ~= 2.77) BBRStartupPacingGain; used in Startup mode for BBR.pacing_gain. <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-05.html#section-2.4-2>
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-05.html#section-5.6.1>
    ...
    /// 0.5, which has been shown to offer good performance when compared with other alternatives. <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-05.html#section-2.4-3>
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-05.html#section-5.6.1>
    ...
    /// equivalent to BBR.state: The current state of a BBR flow in the BBR state machine. <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-05.html#section-2.6-1>
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-05.html#section-3.3>
    state: BbrState,
    ...
    /// `has_seen_rtt`. BBRInitPacingRate uses C.srtt when there is one and 1ms otherwise; the
    /// initial rate here always takes 1ms, and Startup only raises it, so an application-limited
    /// flow that never leaves Startup would keep it for its whole life.
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-5.6.2-6>
    has_seen_rtt: bool,
    ...
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-2.14-5>
    ...
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-2.14-6>
...
impl Bbr3 {
    /// equivalent to BBROnInit <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-05.html#section-5.2.1>
    ...
    /// equivalent to BBRCheckDrainDone <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-5.3.2>
    ///
    /// `Inflight(1.0)` takes BBR.bw, so a loss in Drain that lowers `bw_shortterm` also lowers
    /// the level Drain must reach.
    fn check_drain_done(&mut self, now: Instant) {
        if self.state == BbrState::Drain
            && (self.inflight <= self.get_inflight(self.bw, 1.0)
    ...
    /// equivalent to BBRIsTimeToCruise <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-5.3.3.9>
    ///
    /// The one `Inflight` the draft computes from BBR.max_bw rather than BBR.bw.
    fn maybe_update_budget_and_time_to_cruise(&mut self) -> bool {
        if self.inflight > self.inflight_with_headroom() {
            return false;
        }
        if self.inflight > self.get_inflight(self.max_bw, 1.0) {
            return false;
        }
        true
    }

    /// equivalent to BBRInflight <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-5.6.4.2>
    ///
    /// Takes the bandwidth explicitly, since the draft's callers pass BBR.bw or BBR.max_bw.
    fn get_inflight(&mut self, bw: f64, gain: f64) -> u64 {
        let inflight_cap = self.bdp_multiple(bw, gain);
    ...
    /// equivalent to BBRUpdateMaxInflight <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-5.6.4.2>
    ///
    /// Sized from BBR.bw, so the short-term bandwidth bound caps the window as well as the
    /// pacing rate. As the last BDP computed on each ACK, it also leaves BBR.bdp at
    /// BBR.bw * BBR.min_rtt for `target_inflight`.
    fn update_max_inflight(&mut self) {
        let mut inflight_cap = self.bdp_multiple(self.bw, self.cwnd_gain);
    ...
    /// `bbr_init_pacing_rate_from_rtt`. <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-5.6.2-6>
    ///
...
impl Controller for Bbr3 {
    /// equivalent to BBROnTransmit and OnPacketSent: checks for a restart from idle, then takes
    /// the packet's delivery snapshot.
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-5.2.2>
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-4.1.2.2-4>
    ...
    /// Feeds C.is_cwnd_limited, which the transport provides.
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-2.2-7>
    ...
    ///
    /// Only records this ACK's newest packet; the model waits for `on_end_acks`, once the ACK's
    /// delivered bytes, interval, and inflight are all known.
    fn on_ack(
        &mut self,
        now: Instant,
        sent: Instant,
    ...
    /// congestion, so an ECN event takes the per-loss steps for its largest packet. Persistent
    /// congestion collapses the window to the minimum, here BBR.MinPipeCwnd.
    ...
    /// equivalent to the per-loss steps, which call BBRHandleLostPacket
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-5.2.4>
    fn on_packet_lost(
        &mut self,
        lost_bytes: u16,
        packet_number: u64,
    ...
    /// Re-derives C.SMSS and BBR.MinPipeCwnd (4 * C.SMSS) from the new MTU.
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-2.2-1>
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-06.html#section-2.8-4>
    ...
    /// Records the ACK frequency parameters BBRUpdateOffloadBudget uses for QUIC.
    /// <https://www.ietf.org/archive/id/draft-ietf-ccwg-bbr-05.html#section-5.5.8.2>
...
mod test {
    ...
    impl Sim {
        ...
        fn send(&mut self, now_ns: u64, count: u64) {
            for _ in 0..count {
                ...
                self.bbr
                    .on_packet_sent(now, self.mss as u16, self.pn, SpaceId::Data);
        ...
        fn lose(&mut self, now_ns: u64, pn: u64) {
            ...
            self.inflight -= self.mss;
            self.bbr
                .on_packet_lost(self.mss as u16, pn, SpaceId::Data, self.at(now_ns));
        }
    ...
    /// (`C.inflight <= BBRInflight(1.0)`, i.e. `get_inflight(bw, 1.0)`, the estimated
    ...
    fn drain_exits_to_probe_bw_on_inflight() {
        ...
        // BBRInflight(1.0) == get_inflight(bw, 1.0), round_count).
        ...
        sim.run(
            ...
            |bbr, _now_ns, inflight, _pn| {
                ...
                // Capture the DRAIN -> PROBE_BW edge. get_inflight(bw, 1.0) is
                ...
                if matches!(bbr.state, BbrState::ProbeBw(_)) {
                    let bdp = bbr.get_inflight(bbr.bw, 1.0);
    ...
    /// `C.inflight` stays above `BBRInflight(1.0)` (`get_inflight(bw, 1.0)`) for
            ...
            |bbr, _now_ns, _inflight, _pn| {
                    ...
                    && bbr.state == BbrState::ProbeBw(ProbeBwSubstate::Down)
                {
                    down_entry = Some((
                        ...
                        bbr.get_inflight(bbr.max_bw, 1.0),
                    ));
                }
    ...
    fn probe_bw_exits_probe_down_to_probe_refill_on_max_time() {
        ...
        let cruise_threshold = bbr.get_inflight(bbr.max_bw, 1.0);
        assert!(
    ...
    fn loss_only_event_reuses_no_sample() {
        ...
        sim.bbr.packets[SpaceId::Data as usize].retain(|p| p.packet_number != 2);
        sim.ack(15 * MS, [2], false);
    ...
    /// Holds BBR.bw at `bw_shortterm`, below BBR.max_bw, as a loss round outside probing does.
    fn bound_bw(sim: &mut Sim, bw_shortterm: f64) {
        sim.bbr.bw_shortterm = bw_shortterm;
        sim.bbr.bound_bw_for_model();
    }

    /// A lossy round in PROBE_DOWN lowers BBR.bw, and the window target and BBR.bdp follow it
    /// rather than BBR.max_bw, so `target_inflight` reads the loss-bounded BDP.
    #[test]
    fn loss_bounded_bw_sizes_max_inflight_and_bdp() {
        let mut sim = probed();
        // Lose 10 of the next 100 packets.
        sim.send(10 * MS, 100);
        for pn in 100..110 {
            sim.lose(20 * MS, pn);
        }
        sim.ack(20 * MS, 110..200, false);

        let bbr = &mut sim.bbr;
        assert_eq!(bbr.max_bw, 12_000_000.0);
        assert!(bbr.bw < bbr.max_bw);
        let bdp = (bbr.bw * bbr.min_rtt.as_secs_f64()).round() as u64;
        assert_eq!(bbr.bdp, bdp);
        let expected = bbr.quantization_budget(2 * bdp + bbr.extra_acked);
        assert_eq!(bbr.max_inflight, expected);
        assert_eq!(bbr.target_inflight(), Ord::min(bdp, bbr.cwnd));
    }

    /// DRAIN waits for inflight to reach the BDP at BBR.bw, which a loss in DRAIN lowers below
    /// the BDP at BBR.max_bw.
    #[test]
    fn drain_exit_waits_for_loss_bounded_bdp() {
        let mut sim = scripted();
        // 12 MB/s over a 10ms RTT: a 120 KB BDP at max_bw.
        sim.round(0, 100, 10 * MS);
        sim.bbr.full_bw_reached = true;
        sim.bbr.enter_drain();
        // A 60 KB BDP at bw.
        bound_bw(&mut sim, 6_000_000.0);

        sim.send(10 * MS, 100);
        // 108 KB in flight: below the BDP at max_bw, above the one at bw.
        sim.ack(20 * MS, 100..110, false);
        assert_eq!(sim.bbr.state, BbrState::Drain);
        // 48 KB in flight.
        sim.ack(21 * MS, 110..160, false);
        assert!(matches!(sim.bbr.state, BbrState::ProbeBw(_)));
    }

    /// PROBE_DOWN hands over to CRUISE at the BDP at BBR.max_bw, the one `Inflight` the draft
    /// takes from it, so a lower BBR.bw does not hold DOWN longer.
    #[test]
    fn probe_down_cruises_at_max_bw_bdp() {
        let mut sim = probed();
        bound_bw(&mut sim, 6_000_000.0);

        sim.send(10 * MS, 100);
        // 108 KB in flight: below the 120 KB BDP at max_bw, above the 60 KB one at bw.
        sim.ack(20 * MS, 100..110, false);
        assert_eq!(sim.bbr.state, BbrState::ProbeBw(ProbeBwSubstate::Cruise));
    }

    /// ProbeRTT caps the window at half the BDP at BBR.bw.
    #[test]
    fn probe_rtt_cwnd_follows_loss_bounded_bw() {
        let mut sim = probing_rtt();
        bound_bw(&mut sim, 6_000_000.0);
        sim.bbr.set_cwnd(0);
        // Half of the 60 KB BDP at bw, not of the 120 KB one at max_bw.
        assert_eq!(sim.bbr.cwnd, 30_000);
    }
    ...
    fn probe_rtt_protects_its_samples() {
        ...
        let newest_limited = |sim: &Sim| {
            sim.bbr.packets[SpaceId::Data as usize]
                .back()
                .unwrap()
                .is_app_limited
        };
    ...
    fn ack_samples_the_packet_from_its_own_space() {
        ...
        c.on_packet_sent(at(0), PACKET, 0, SpaceId::Initial);
        c.on_packet_sent(at(1), PACKET, 1, SpaceId::Initial);
        c.on_ack(
            at(10),
            at(0),
            PACKET as u64,
        ...
        c.on_end_acks(at(10), PACKET as u64, false, Some(0), SpaceId::Initial);
        // Sent after Initial 0 was delivered, so its snapshot counts that delivery.
        c.on_packet_sent(at(11), PACKET, 0, SpaceId::Handshake);
        c.on_ack(
            at(20),
            at(11),
            PACKET as u64,
    ...
    fn loss_removes_the_packet_from_its_own_space() {
        ...
        c.on_packet_lost(PACKET, 0, SpaceId::Handshake, at(10));
