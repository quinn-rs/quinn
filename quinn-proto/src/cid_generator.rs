...
pub trait ConnectionIdGenerator: Send + Sync {
    ...
    /// Generates a new CID for a connection with a peer at `remote`
    ///
    /// Defaults to [`generate_cid`](Self::generate_cid), which is the right
    /// behavior for generators that do not care who the peer is.
    ///
    /// Override this to bind the peer's address into the CIDs an endpoint
    /// issues, as address-aware load balancers and packet-level authentication
    /// schemes need to. Note that a peer that migrates will keep using CIDs
    /// issued for its previous address until it is issued new ones, and that
    /// `local_ip` is only populated on platforms that report the destination
    /// address of a received datagram.
    ///
    /// The requirements documented on [`generate_cid`](Self::generate_cid)
    /// apply here too: whatever is encoded must still be opaque and
    /// high-entropy to an observer who does not hold the issuer's secrets.
    fn generate_cid_for(&mut self, _remote: SocketAddr, _local_ip: Option<IpAddr>) -> ConnectionId {
        self.generate_cid()
    }
