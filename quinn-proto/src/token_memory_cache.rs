...
impl<const N: usize> State<N> {
    ...
    fn store(&mut self, server_name: &str, token: Bytes) {
        ...
        if N == 0 {
            // similarly to above, the in-place queue is not designed to work with a length of 0,
            // so avoid ever constructing it in that case.
            return;
        }
...
mod tests {
    ...
    fn zero_max_server_names() {
        ...
        const N: usize = 2;
        let cache: TokenMemoryCache<N> = TokenMemoryCache::new(0);
        for i in 0..10 {
    ...
    fn zero_queue_length() {
        // test that this edge case doesn't panic
        const N: usize = 0;
        let cache: TokenMemoryCache<N> = TokenMemoryCache::new(256);
        for i in 0..10 {
            cache.insert(&i.to_string(), Bytes::from(vec![i]));
            for j in 0..10 {
                assert!(cache.take(&j.to_string()).is_none());
