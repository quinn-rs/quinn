...
#[cfg(feature = "trace")]
use ant_quic::tracing::{EventLog, TraceId, Event, EventData};
#[cfg(not(feature = "trace"))]
use ant_quic::tracing::{EventLog, TraceId, Event};
use std::sync::Arc;
use std::thread;
...
fn main() {
    ...
    #[cfg(feature = "trace")]
    {
        ...
        for i in 0..5 {
            ...
            log.log(event);
            println!("  - Logged packet received: size={}, num={}", 1200 + i * 100, i);
        }
        ...
        for (i, event) in events.iter().enumerate() {
            ...
            match &event.event_data {
                ...
                EventData::PacketSent { size, packet_num, .. } => {
                    println!("Packet Sent (size={}, num={})", size, packet_num);
                }
                EventData::PacketReceived { size, packet_num, .. } => {
    ...
    #[cfg(not(feature = "trace"))]
    {
        ...
        println!("  size_of::<TraceId>() = {}", std::mem::size_of::<TraceId>());
        println!("  size_of::<Event>() = {}", std::mem::size_of::<Event>());
        println!("  size_of::<EventLog>() = {}", std::mem::size_of::<EventLog>());
    }
}