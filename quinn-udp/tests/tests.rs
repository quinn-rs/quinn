...
/// 2. After calling `enable_apple_fast_path()`, `max_gso_segments()` returns `BATCH_SIZE`
...
fn apple_fast_datapath() {
    ...
    send_state.enable_apple_fast_path();
    recv_state.enable_apple_fast_path();

    // After enabling, fast path should be enabled and max_gso_segments should be BATCH_SIZE
    assert!(
        send_state.is_apple_fast_path_enabled(),
        "fast path should be enabled after calling enable_apple_fast_path()"
    );
    assert_eq!(
        send_state.max_gso_segments(),
