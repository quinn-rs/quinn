...
impl PeerStore {
    ...
    fn resolve_geo(&self, addresses: &[SocketAddr]) -> (f64, f64, Option<String>) {
        ...
        if ip_str.starts_with("77.42.")
            || ip_str.starts_with("95.216.")
            || ip_str.starts_with("65.109.")
        {
            // Hetzner Helsinki
            return (60.1699, 24.9384, Some("FI".to_string()));
        }
        if ip_str.starts_with("5.9.")
            || ip_str.starts_with("78.46.")
            || ip_str.starts_with("88.99.")
        {
            // Hetzner Falkenstein
            return (50.4779, 12.3713, Some("DE".to_string()));
        }
        if ip_str.starts_with("138.201.")
            || ip_str.starts_with("148.251.")
            || ip_str.starts_with("144.76.")
        {
            // Hetzner Nuremberg
        ...
        if ip_str.starts_with("159.65.")
            || ip_str.starts_with("164.90.")
            || ip_str.starts_with("167.99.")
        {
            // DigitalOcean various (default to NYC)
            return (40.7128, -74.0060, Some("US".to_string()));
        }
        if ip_str.starts_with("162.243.")
            || ip_str.starts_with("104.131.")
            || ip_str.starts_with("192.241.")
        {
            // DigitalOcean NYC
            return (40.7128, -74.0060, Some("US".to_string()));
        }
